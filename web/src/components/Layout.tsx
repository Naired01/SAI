...
import {
  ...
  ListChecks, KeyRound, LogOut, Languages,
} from 'lucide-react'
      ...
      <aside className="w-60 shrink-0 bg-white border-r border-slate-200 flex flex-col">
        <div className="px-5 py-4 border-b border-slate-200">
          <Link to="/" className="block">
            <div className="text-xl font-bold text-brand-700">SAI</div>
        ...
        <div className="p-3 border-t border-slate-200 space-y-1">
          <button onClick={toggleLang} className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-slate-700 hover:bg-slate-100">
            <Languages size={18} /> {lang.toUpperCase()}
          </button>
          <div className="px-3 py-2 text-xs text-slate-500 truncate" title={user?.email}>{user?.email}</div>
          <button onClick={onLogout} className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-red-700 hover:bg-red-50">
